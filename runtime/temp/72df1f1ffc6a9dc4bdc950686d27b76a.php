<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:82:"/www/wwwroot/xiaoyun.wutao7.top/public/../application/service/view/chat/index.html";i:1640222864;}*/ ?>
...
<script type="text/javascript" src="/assets/libs/webrtc/recorder.wav.min.js"></script>
...
<script type="application/javascript">
    ...
    , authEndpoint:  '/admin/login/auth'
        ...
        channel.bind("cu-event", function (data) {
            ...
            if(data.message.content.indexOf('<img')>= 0||data.message.content.indexOf('<a')>= 0||!isNaN(data.message.content)||data.message.content.indexOf('<video')>= 0||!isNaN(data.message.content)||data.message.content.indexOf('<audio')>= 0){
            ...
            if (data.message.visiter_id == debug) {
                ...
                str =str.replace(/api.map.baidu.com/gi,'[定位]');
                str =str.replace(/<audio src=['"]([^'"]+)[^>]*>/gi,'[语音]');
            ...
            <div class="chatbox hide" style="width: 100%;height: 100%;padding-bottom: 242px">
                ...
                <div class="footer">
                    ...
                    <div class="msg-toolbar" style="background: #fff;border: none;">
                        ...
                        <?php endif; ?>
                        <a onclick="recOpen()" title="发送语音"><i class="layui-icon" style="font-size: 22px;cursor: pointer;margin-top: -6px;display: inline-block;">&#xe688;</i></a>
...
<script type="text/javascript">
    ...
    function showPosition(position){
        ...
        $.ajax({
            url:"/admin/set/uploadLocation",
            type: "post",
            data: {visiter_id:sid,location:lat+','+lag,unstr:unstr},
            dataType:'json',
            success:function(res){
                if(res.code != 0){
                    layer.msg(res.msg,{icon:2});
                    ...
                    str += "<pre>";
                    str += "<a href='http://api.map.baidu.com/geocoder?location="+lat+","+lag+"&output=html&src=webapp.baidu.openAPIdemo' style='display: block;' target='_blank'><p style='margin-bottom: 10px'>" + res.data + "</p><img src='/assets/images/admin/B/map.png'/></a><span onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</span>";
                        ...
                        rec.stop(function(blob,duration){
                            ...
                            xhr.onreadystatechange = function () {
                                if (xhr.readyState == 4 && xhr.status == 200) {
                                    ...
                                    str += '<div style="position: absolute;top: 26px;right: 2px;"><img  class="my-circle se_pic" src="' + pic + '" width="50px" height="50px"></div>';
                                    str += "<div class='outer-right'><div class='service'>";
                                    str += "<pre>" +  voicemessage + "<span onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</span></pre>";
                                    str += "</div></div>";
                                    str += "</li>";
